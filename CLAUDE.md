...
│   └── fix-keywords.js         # 重跑医生名+国家/地区+医院名关键词
└── src/
    ├── index.js                # Express 服务入口
    ├── config.js               # 端口、数据目录、Puppeteer 配置
    ├── scraper.js              # Puppeteer 抓取：标题、摘要、来源、日期
    ├── keywords.js             # 关键词提取：医生名（中/英）+ 国家/地区 + 医院名
...
| 关键词提取 | 正则匹配医生名（中/英）+ 国家/地区 + 医院名；@node-rs/jieba 仅作兜底 |
...
5. 关键词提取（[src/keywords.js](src/keywords.js)）：医生姓名 → 国家/地区 → 医院名称
   - 中文医生名：2-3 字姓名紧贴头衔词（教授/主任医师/博士/医师等）作锚点，再剥除被贪心匹配带上的部门/连接字
   - 英文医生名：首字母大写的姓名序列（Kenneth C. Anderson、Wee Joo Chng、Dr.Chutima Kunacheewa），支持连字符姓氏与全大写姓
   - 国家/地区：内置词典按别名归一（沙特→沙特阿拉伯、香港→中国香港），**刻意不含「中国」**（50/104 篇都有，无区分度）
...
以「医生姓名 + 国家/地区 + 医院名称」为关键词主体，聚焦"谁在分享、来自哪家医院"：

```js
// 中文医生名：头衔词作锚点（贪婪会带上前面字，需剥前缀，含标题分隔符「丨」）
const CN_DOCTOR = /[一-鿿]{1,3}(?:教授|主任医师|主治医师|医师|博士|院长|研究员|导师)/g;
// name = token 去头衔后，剥除部门/连接前缀（院/科/邀/由/丨…），黑名单过滤伪名

// 英文医生名：首字母大写姓名序列，支持 Dr./Prof. 前缀、全大写姓（CHNG）、连字符姓（Al-Shemmari）
const EN_NAME_TOKEN =
  /(?:[A-Z][a-z]+(?:-[A-Z][a-z]+)?(?:\s+(?:[A-Z]\.|[A-Z][a-z]+(?:-[A-Z][a-z]+)?|[A-Z]{2,}))+|...)/g;

// 国家/地区：词典 + 别名归一（沙特→沙特阿拉伯、香港→中国香港），刻意不含「中国」
const COUNTRY_ALIASES = [["沙特阿拉伯", ["沙特阿拉伯", "沙特"]], ["中国香港", ["中国香港", "香港"]], ...];
// 按别名长度降序匹配，命中后等长挖空，避免「印度尼西亚」里再匹配出「印度」

// 医院名：后缀锚定后，在连续汉字串里向右回退到分隔字（于/在/由/…）之后
// 注意1：紧贴后缀的字（协和|医院 的"和"）属于医院名，不可当分隔符
// 注意2：「中国医院」这类泛指用 HOSP_GENERIC 黑名单排除
const HOSP_SUFFIX = /(?:医院|医学院|医学中心|研究所|研究院|...)$/;

// 排序：标题(医生→国家→医院) → 正文(医生→国家→医院)；子串去重（国家豁免）；上限 10
// 无结构化匹配时才 jieba 兜底，且正文 < 50 字不兜底
```
...
node scripts/fix-keywords.js   # 重跑医生名+国家/地区+医院名关键词
...
- **需求驱动**：早期用 jieba 分词 + n-gram，产出大量碎片（"多发性骨""爱斌教授""江大学医"），无法点击检索到"谁在分享、来自哪家医院"。改版后关键词以医生全名 + 国家/地区 + 医院全名为准。
- **中文医生名**：以头衔词（教授/主任医师/博士…）作锚点反推姓名，天然避开分词难题。但贪婪匹配会把前面的部门字带上（"医院血液科冯茹教授" → 抓到"科冯茹"），需要剥除前缀垃圾字（院/科/邀/由…）。**标题分隔符 `丨` 也必须剥**（"大CAR愈见丨安刚教授"），否则 `丨安刚` 与 `安刚` 同时产出，子串去重会误删干净的那个。
- **英文医生名**：首字母大写的姓名序列，需兼容 `Dr.X`（无空格）、全大写姓（`Wee Joo CHNG`）、**连字符姓氏**（`Al-Shemmari`；不支持连字符时会被截断成 `Salem H. D. Al`）、教授/学位尾缀；用常见学术词停用表（Antigen、Year、Session…）过滤非人名短语。
- **国家/地区**：内置词典 + 别名归一。三个关键设计：
  1. **刻意不含「中国」**——它出现在 50/104 篇文章里，作为关键词没有区分度。
  2. **别名归一**：沙特→沙特阿拉伯、香港→中国香港；港澳台按「中国XX」与主权国家一同提取。
  3. **长名前缀优先 + 等长挖空**：按别名长度降序匹配，命中后把该片段挖掉（等长填充、位置不变），否则「印度尼西亚」里会再匹配出一个「印度」。
- **医院名**：以医院/医学院/研究所后缀锚定。两个关键坑：
  1. 医院名内部可能含连词字："协和医院"的"和"、以及"中"（中国医学科学院）等，不能一律当分隔符；正确做法是取连续汉字串里**分隔字之后、且紧贴后缀的分隔字跳过**的整段。
  2. **泛指不是机构**：「前往中国医院参观」里的"中国医院"是"中国 + 医院"泛指，需用 `HOSP_GENERIC` 黑名单排掉。
- **排序**：标题优先（核心汇报人 / 主办地），再正文；每层内固定 **医生名 → 国家/地区 → 医院名**，结构去重后截 10 个。
- **子串去重**：「华西」⊂「华西医院」时只留长者；但**国家/地区豁免**，否则「美国」会被「美国哈佛医学院」吞掉。
- **兜底**：正文确无结构化匹配时才回退 jieba；且**正文短于 50 字就不兜底**——空壳文章（正文是「文章详情」这类占位符）切出来全是「倾听/临床/声音」泛词，留空反而能暴露数据问题。
- **脚本**：`node scripts/fix-keywords.js` 用正文重跑全部文章，无需重新抓取。注意其判断条件是 `kw !== oldKw` 而非 `kw && kw !== oldKw`，否则**空结果写不回去、旧噪音清不掉**。
